...
void QgsCollapsibleGroupBoxBasic::updateStyle()
{
  ...
  int offsetStyle = qApp->style()->objectName().contains( "macintosh" ) ? ( usingQgsStyle ? 1 : 8 ) : 0;
...
//    offsetTopTri += rectTitle.top();
  ...
  if ( qApp->style()->objectName().toLower() == "oxygen" )
  {
    QStyleOptionGroupBox box;
    initStyleOption( &box );
    QRect rectFrame = style()->subControlRect( QStyle::CC_GroupBox, &box,
  ...
  QgsDebugMsg( QString( "groupbox: %1 style: %2 offset: left=%3 top=%4 top2=%5" ).arg(
                 objectName() ).arg( qApp->style()->objectName() ).arg( offsetLeft ).arg( offsetTop ).arg( offsetTopTri ) );

  // customize style sheet for collapse/expand button and force left-aligned title
  QString ss;
  if ( usingQgsStyle || qApp->style()->objectName().contains( "macintosh" ) )
  {
  ...
  if ( qApp->style()->objectName().contains( "macintosh" ) )
  {
