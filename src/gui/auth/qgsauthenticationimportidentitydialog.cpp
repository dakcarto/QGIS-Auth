                                                   ...
                                                   bool append )
{
  ...
  {
    case Valid:
      ss = QgsAuthCertUtils::greenTextStyleSheet( "QTextEdit" );
      txt = tr( "Valid: %1" ).arg( msg );
      break;
    case Invalid:
      ss = QgsAuthCertUtils::redTextStyleSheet( "QTextEdit" );
...
bool QgsAuthImportIdentityDialog::validatePkiPaths()
{
  ...
  {
    writeValidation( tr( "Multiple certificates in file" ), Invalid );
    return false;
  }
  else
  {
