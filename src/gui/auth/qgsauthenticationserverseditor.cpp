                                                   ...
                                                   QTreeWidgetItem *parent )
{
  ...
  Q_FOREACH( QgsAuthConfigSslServer config, configs )
  {
    ...
    coltxts << QString( config.sslHost() );
...
void QgsAuthServersEditor::on_btnRemoveServer_clicked()
{
  ...
  if ( !QgsAuthManager::instance()->existsSslCertCustomConfig( digest ) )
  {
  ...
  if ( !QgsAuthManager::instance()->removeSslCertCustomConfig( digest ) )
  {
    messageBar()->pushMessage( tr( "ERROR removing SSL custom config from authentication database for id %1:" ).arg( digest ),
...
void QgsAuthServersEditor::on_btnEditServer_clicked()
{
  ...
  QgsAuthConfigSslServer config( QgsAuthManager::instance()->getSslCertCustomConfig( digest ) );
  QSslCertificate cert( config.sslCertificate() );

  QgsAuthSslConfigDialog * dlg = new QgsAuthSslConfigDialog( this, cert );
